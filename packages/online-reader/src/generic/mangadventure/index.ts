...
import { DEFAULT_CATEGORIES, FilterControlIds, SORT_FIELDS, STATUS_OPTIONS } from './filters';
...
export class MangAdventureClient {
  ...
  getDirectory: GetDirectoryFunc = (page, filterValues) => this.getSearch('', page, filterValues);
  ...
  getSearch: GetSearchFunc = (text, page, filterValues) => {
    ...
    if (FilterControlIds.Categories in filterValues) {
      ...
      const categories = Object.entries(values)
        .filter(([_key, value]) => value != TriState.IGNORE)
  ...
  getPageUrls: GetPageUrlsFunc = (pageRequesterData) => pageRequesterData.pageFilenames;

  getImage: GetImageFunc = (_series, url) => Promise.resolve(url);
  ...
  getFilterOptions: GetFilterOptionsFunc = () => [
    ...
    new FilterSelect(FilterControlIds.Status, 'Status', 'any').withOptions(STATUS_OPTIONS),
  ...
  setSettings: SetSettingsFunc = (_newSettings) => {};
