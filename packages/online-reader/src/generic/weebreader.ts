...
export class WeebReaderClient {
  ...
  getSeries: GetSeriesFunc = (id: string) => {
    return fetch(`${this.baseUrl}/api/titles/${id}`)
  ...
  getPageRequesterData: GetPageRequesterDataFunc = (
    ...
    return fetch(`${this.baseUrl}/api/chapters/${chapterSourceId}`)
      .then((response: Response) => response.json())
      .then((data: any) => {
  ...
  getSearch: GetSearchFunc = (text: string, _page: number) => {
    return fetch(`${this.baseUrl}/api/titles/search?term=${text}`)
      .then((response: Response) => response.json())
      .then((data: any) => {
  ...
  getDirectory: GetDirectoryFunc = (_page: number) => {
    return fetch(`${this.baseUrl}/api/titles`)
      .then((response: Response) => response.json())
      .then((data: any) => {
        const seriesList = this._parseSeriesList(data);
        return {
