...
export class MadaraClient {
  ...
  getChapters: GetChaptersFunc = (id: string) => {
    return fetch(`${this.baseUrl}${id}/ajax/chapters`, { method: 'POST' })
      .then((response) => response.text())
      .then((text) => {
  ...
  getPageRequesterData: GetPageRequesterDataFunc = (
    ...
    return fetch(`${this.baseUrl}/${seriesSourceId}/${chapterSourceId}`)
      .then((response) => response.text())
      .then((text) => {
        const doc = new JSDOM(text).window.document;
  ...
  getSearch: GetSearchFunc = (text: string, page: number, _filterValues: FilterValues) => {
    return fetch(`${this.baseUrl}/page/${page}/?s=${text}&post_type=wp-manga`, {
      headers: this.searchHeaders,
    })
      .then((response) => response.text())
      .then((text) => this._parseSearch(new JSDOM(text).window.document));
  };
