...
export class NepClient {
  ...
  _getDirectoryList = () => {
    return this.webviewFn(`${this.baseUrl}/search`).then((response: WebviewResponse) => {
  ...
  _decodeChapterId = (id: string): { path: string; number: number } => {
    ...
    const n = id.substring(dgt, id.length - 1);
    let suffix = '';
    const path = id.substring(id.length - 1);
  ...
  getSeries: GetSeriesFunc = (id: string) => {
    return this.webviewFn(`${this.baseUrl}/manga/${id}`).then((response: WebviewResponse) => {
      ...
      const doc = new JSDOM(fixedData).window.document;
  ...
  getPageRequesterData: GetPageRequesterDataFunc = (
    ...
    return this.webviewFn(`${this.baseUrl}/read-online/${seriesSourceId}${chapterSourceId}`).then(
  ...
  setSettings: SetSettingsFunc = (_newSettings: { [key: string]: any }) => {};
